/**
 * BIP-39 English wordlist, verbatim (2048 words).
 *
 * Chosen over a hand-rolled list for three reasons that matter for a fingerprint:
 *   - 2048 words = 11 bits each, so eight words carry 88 bits.
 *   - Every word is uniquely identified by its first four letters, which makes it robust to
 *     mishearing and to sloppy spelling when someone reads it down a phone line.
 *   - No two words in the list are confusable homophones, and it is already audited and
 *     widely reviewed — not something I should be inventing.
 *
...
/** 2048 = 2^11. Asserted at module load so a bad edit fails loudly instead of silently
 *  biasing word selection. */
export const BITS_PER_WORD = 11;

if (WORDS.length !== 1 << BITS_PER_WORD) {
  throw new Error(`wordlist must be ${1 << BITS_PER_WORD} words, got ${WORDS.length}`);
}
